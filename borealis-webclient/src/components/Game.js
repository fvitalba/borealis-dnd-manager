...
const Game = () => {
	const renderCursors = () => {
		...
		const cursors = Object.assign({}, cursors)
		...
		return (
			<div id='cursors'>
				{ Object.keys(cursors).map((key, $i) => (
					<Cursor key={ `cursor${$i}` } name={ key } cursor={ cursors[key] } size={ cursorSize } />
				)) }
			</div>
		)
	}
	...
	// { game, state, onMouseMove, onMouseUp, onMouseDown, renderTokens, renderCursors, backgroundRef, drawingRef, fogRef, overlayRef, controlPanelRef, handleError }
	return (
		<GameView 

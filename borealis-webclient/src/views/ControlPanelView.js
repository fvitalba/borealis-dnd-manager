...
const ControlPanelView = ({ controlPanelState, setControlPanelState, hidden, toggleHidden, fogEnabled, isHost, username, setUsername, cursorSize, setCursorSize, socketRequestRefresh, pushRefreshToPlayers }) => {
    if (hidden)
        return (
            <div id='control-panel' className={ controlPanelClass }>
                <Button value={ <Show width='24' /> } onClick={ toggleHidden } title='show/hide control panel' />
            </div>
        )
    ...
    if (isHost)
        return (
            <div id='control-panel' className={ controlPanelClass }>
                <Button value={ <Show width='24' /> } onClick={ toggleHidden } title='show/hide control panel' />
                <ToggleButton title='User' value={ <UsersCog width='24'/> } controlPanelState={ controlPanelState } setControlPanelState={ setControlPanelState } />
                <ToggleButton title='Maps' value={ <Map width='24' />} controlPanelState={ controlPanelState } setControlPanelState={ setControlPanelState } />
                <ToggleButton title='Tokens' value={ <Chess width='24' /> } controlPanelState={ controlPanelState } setControlPanelState={ setControlPanelState } />
                <Button title='Push refresh to players' value={ <RefreshCircle width='24' /> } onClick={ pushRefreshToPlayers } />
